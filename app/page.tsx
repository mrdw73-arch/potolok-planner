...
export default function Home() {
  ...
  function currentProject(): CeilingProject {
    return { id: projectId || createProjectId(), name: name.trim() || 'Без названия', updatedAt: new Date().toISOString(), points, elements, prices: defaultPrices, client, notes, variants: variants.map(v => ({ id: v.id, name: v.name, price: v.id === activeVariant ? variantPrice : v.price })), activeVariantId: activeVariant, quantityOverrides: overrides, orthogonalMode };
  }
  ...
  function movePoint(i: number, e: PointerEvent<SVGCircleElement>) {
    ...
    const target = snapPoint({ x: Math.max(0, (p.x - bounds.ox) / bounds.scale + bounds.minX), y: Math.max(0, (p.y - bounds.oy) / bounds.scale + bounds.minY) }, 10);
    setPoints(current => {
      if (!orthogonalMode || current.length !== 4) return current.map((v, j) => j === i ? target : v);
      const result = current.map(v => ({ ...v }));
      const prev = (i + 3) % 4;
      const next = (i + 1) % 4;
      const opposite = (i + 2) % 4;
      result[i] = target;
      result[prev].y = target.y;
      result[next].x = target.x;
      result[opposite] = { x: result[next].x, y: result[prev].y };
      return result;
    });
  }
  function setWallLength(index: number, lengthMm: number) {
    if (!Number.isFinite(lengthMm) || lengthMm <= 0) return;
    const length = Math.round(lengthMm / 10) * 10;
    setPoints(current => orthogonalMode && current.length === 4 ? resizeOrthogonalWall(current, index, length) : resizeWallKeepingAdjacent(current, index, length));
  }
