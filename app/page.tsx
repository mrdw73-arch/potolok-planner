...
export default function Home() {
  ...
  function loadProject(project: CeilingProject) {
    setProjectId(project.id); setName(project.name); setPoints(project.points); setElements(project.elements); setClient(project.client); setClientId(project.clientId); setNotes(project.notes);
    setOverrides(project.quantityOverrides ?? {});
    setOrthogonalMode(project.orthogonalMode ?? false);
    ...
    setNextId(Math.max(0, ...project.elements.map(e => e.id)) + 1); setSelectedPoint(0); setSelectedWall(0); setSelectedElement(null); setEditingWall(null); setStatus('Проект открыт');
  }

  function currentProject(): CeilingProject {
    return { id: projectId || createProjectId(), name: name.trim() || 'Без названия', updatedAt: new Date().toISOString(), points, elements, prices: defaultPrices, client, clientId, notes, variants: variants.map(v => ({ id: v.id, name: v.name, price: v.id === activeVariant ? variantPrice : v.price })), activeVariantId: activeVariant, quantityOverrides: overrides, orthogonalMode };
  }
  ...
  function newProject() {
    setProjectId(''); setName('Новая комната'); setPoints(initialPoints); setElements([]); setClient({ name: '', phone: '', address: '' }); setClientId(undefined); setNotes(''); setOverrides({}); setVariants([{ id: 'standard', name: 'Стандарт', price: 0, active: true }]); setActiveVariant('standard'); setNextId(1); setSelectedPoint(0); setSelectedWall(0); setSelectedElement(null); setEditingWall(null); setShowDiagonals(true); setOrthogonalMode(false); setStatus('Новый проект');
  }

  function selectClient(id: string) {
    if (!id) { setClientId(undefined); return; }
    const found = clients.find(c => c.id === id);
    if (!found) return;
    setClient({ name: found.name, phone: found.phone, address: found.address });
    setClientId(found.id);
  }

  function saveClientToDatabase() {
    if (!client.name.trim()) { setStatus('Укажите имя клиента'); return; }
    const existing = clientId ? clients.find(c => c.id === clientId) : undefined;
    const saved: Client = { id: existing?.id ?? createClientId(), name: client.name.trim(), phone: client.phone.trim(), address: client.address.trim(), notes: existing?.notes ?? '', updatedAt: new Date().toISOString() };
    const next = upsertClient(saved, clients);
    setClients(next); setClientId(saved.id); setStatus('Клиент сохранён в базе');
  }

  function saveClient(record: Client) {
    const next = upsertClient(record, clients);
    setClients(next);
  }

  function deleteClient(id: string) {
    const next = removeClientLocally(id, clients);
    setClients(next);
    if (clientId === id) setClientId(undefined);
  }

  function toggleOrthogonalMode(enabled: boolean) {
    setOrthogonalMode(enabled);
    if (enabled && points.length === 4) {
      setPoints(current => orthogonalizeRoom(current));
      setStatus('Ортогональный режим включён');
    } else if (!enabled) {
      setStatus('Свободная геометрия');
    }
  }

  function removeProject(id: string) {
    const next = removeProjectLocally(id, projects); setProjects(next); if (id === projectId) newProject();
  }

  function duplicateCurrent() {
    const project = duplicateProject(currentProject());
    const next = upsertProject(project, projects);
    setProjects(next); loadProject(project);
  }

  function exportBackup() {
    const data = exportProjectsJson(projects);
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `potolok-planner-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  function importBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        const next = importProjectsJson(String(reader.result), projects);
        setProjects(next);
        setStatus('Резервная копия импортирована');
      } catch (error) {
  ...
  function movePoint(index: number, e: PointerEvent<SVGCircleElement>) {
    ...
    const raw = { x: (sx - bounds.ox) / bounds.scale + bounds.minX, y: (sy - bounds.oy) / bounds.scale + bounds.minY };
    const snapped = snapPoint(raw, 10);
    setPoints(current => current.map((point, pointIndex) => pointIndex === index ? snapped : point));
  }

  function setWallLength(index: number, value: number) {
    if (!Number.isFinite(value) || value <= 0) return;
    setPoints(current => orthogonalMode && current.length === 4 ? resizeOrthogonalWall(current, index, value) : resizeWallKeepingAdjacent(current, index, value));
  }

  function beginWallEdit(index: number) {
    setEditingWall(index);
    setEditingWallValue(String(Math.round(distance(points[index], points[(index + 1) % points.length]))));
  }
  function commitWallEdit() {
    if (editingWall === null) return;
    const value = Number(editingWallValue);
    if (Number.isFinite(value) && value > 0) setWallLength(editingWall, value);
    setEditingWall(null);
  }
