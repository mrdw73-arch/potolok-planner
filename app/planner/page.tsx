...
export default function PlannerWorkspace() {
  ...
  const estimate = useMemo(() => calculateEstimate(points, items, {
    canvasPricePerM2: 900, profilePricePerM: 350, insertPricePerM: 120, fastenerPricePerM: 45,
    spotlightPrice: 700, chandelierPrice: 1200, lightLinePricePerM: 950, cornicePricePerM: 650,
    wastePercent: 7, laborPricePerM2: 500,
  }), [points, items]);
  ...
  const updateDiagonal = (id: number, value: string) => {
    const mm = parseDimension(value); if (!mm || mm <= 100) return;
    const diagonal = diagonals.find(d => d.id === id); if (!diagonal) return;
    const result = rebuildQuadrilateralByDiagonal(points.map(p => ({ x: p.x * 10, y: p.y * 10 })), mm, diagonal.a === 0 ? 0 : 1);
    if (!result.ok) { window.alert(result.reason); return; }
    commit(result.points.map(p => ({ x: p.x / 10, y: p.y / 10 })));
    setDiagonals(ds => ds.map(d => d.id === id ? { ...d, lengthMm: mm } : d));
    setDiagonalLength(String(Math.round(mm)));
  };
